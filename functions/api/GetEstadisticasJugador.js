...
export async function onRequestPost(context) {
  const limited = await applyRateLimit(context.request, context.env, 'GetEstadisticasJugador');
  if (limited) return withCors(context.request, limited, context.env);
  const response = await proxyLegacyRequest(context.request, 'GetEstadisticasJugador', 30);
