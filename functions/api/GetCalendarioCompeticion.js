...
export async function onRequestPost(context) {
  const limited = await applyRateLimit(context.request, context.env, 'GetCalendarioCompeticion');
  if (limited) return withCors(context.request, limited, context.env);
  const response = await proxyLegacyRequest(context.request, 'GetCalendarioCompeticion', 120);
